...
export function ComplianceModal({ isOpen, onClose, onConfirm, providerName }: ComplianceModalProps) {
  ...
  const handleConfirm = () => {
    if (canProceed) {
      localStorage.setItem('hyperrouter_compliance_agreed', 'true');
      onConfirm();
    }
  };
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div 
            ...
            className="relative bg-[#16161a] border border-white/10 rounded-2xl shadow-2xl max-w-md w-full p-6 flex flex-col gap-5"
          >
            {/* Header */}
            ...
            <div className="flex flex-col gap-4">
              ...
              >
                <div className={`mt-0.5 w-5 h-5 rounded flex-shrink-0 flex items-center justify-center border transition-all ${aupChecked ? 'bg-teal-500 border-teal-500' : 'bg-white/5 border-white/20 group-hover:border-white/40'}`}>
              ...
              >
                <div className={`mt-0.5 w-5 h-5 rounded flex-shrink-0 flex items-center justify-center border transition-all ${amlChecked ? 'bg-teal-500 border-teal-500' : 'bg-white/5 border-white/20 group-hover:border-white/40'}`}>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {t('compliance.proceed').replace('{provider}', providerName)}
              </button>
            </div>
          </motion.div>
        </div>
